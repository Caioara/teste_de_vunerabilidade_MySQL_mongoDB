...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Gera graficos PNG a partir do CSV de benchmark do MySQL."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Caminho do CSV gerado pelo mysql_benchmark.py",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
...
def plot_resources(resource_by_volume, output_path):
    ...
    plt.title("MySQL - Consumo medio de recursos")
    fig.tight_layout()
    plt.savefig(output_path, dpi=150)
    plt.close(fig)
...
def main():
    ...
    plot_line(
        ...
        "tempo",
        "Tempo medio (ms)",
        output_dir / "mysql_tempo_medio.png",
    )
    plot_line(
        by_operation,
        "throughput",
        "Throughput medio (ops/s)",
        output_dir / "mysql_throughput_medio.png",
    )
    if resource_by_volume:
        plot_resources(
            resource_by_volume,
            output_dir / "mysql_recursos_medios.png",
        )
